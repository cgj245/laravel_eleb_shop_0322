...
@section('contents')
    ...
    <form action="{{route('menus.update',[$menu])}}" method="post" enctype="multipart/form-data" style="width: 400px">
        ...
        <div class="form-group">
            <label>名称:</label>
            <input type="text" class="form-control" name="goods_name" value="{{$menu->goods_name}}" placeholder="请输入名称">
        </div>
        <div class="form-group">
            <label>价格:</label>
            <input type="text" class="form-control" name="goods_price" value="{{$menu->goods_price}}" placeholder="请输入价格">
        </div>
        <div class="form-group">
            <label>描述:</label>
            <textarea class="form-control" name="description"  cols="30" rows="10">{{$menu->description}}</textarea>
        </div>
        <div class="form-group">
            <label>菜品分类:</label>
